...
<?php
 ...
 if(isset($_REQUEST['paging']))
        ...
        $sql = "SELECT * from notice where dept_id='$_REQUEST[id]' AND status='active' ";
        //echo $sql;
      ...
      if(isset($_REQUEST['name'])){
        $sql="select * from notice where dept_id LIKE '%$_REQUEST[id]%'  AND status='active' order by id desc limit $startfrom, $listing_per_page";
		//echo $sql;
...
?>
 ...
 <table width="650px" style="margin-top:10px;font-size:13px;border-radius:15px;" align="center" >
		...
		{  $n++;
			...
			<td style="text-align:left;width:600px;"><a href="view_notice.php?t_id=<?php echo $row['t_id'];?>& semester=<?php echo $row['semester'];?>& year=<?php echo $row['year'];?>& dept_id=<?php echo $row['dept_id'];?>& course_code=<?php echo $row['course_code'];?>">
				<?php echo $row['topic']; 
		...
		<form action="dept_notice.php?id=<?php echo $_REQUEST['id']; ?>" name="form1" method="post" style="margin-left:675px;">
