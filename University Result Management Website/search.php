...
<?php
	include("include/connection.php");
		...
		$s_sql="select * from students order by id asc ";
		$s_res=mysql_query($s_sql);
		$fl=0;
		while($ch=mysql_fetch_array($s_res))
		{
			if($ch['s_id']==$s_id && $ch['status']=="active")
			{
				$fl=1;
				break;
			}
			if($ch['s_id']==$s_id && $ch['status']=="inactive")
			{
				$fl=2;
				break;
			}
...
<?php
		...
		$s_sql="select * from students where s_id='$s_id'  ";
		$s_res=mysql_query($s_sql);
		$s_arr=mysql_fetch_array($s_res);
		$s_year=$s_arr['admission_year'];
		for($i=$s_year;$i<=($s_year+5);$i++)
		{
		$fl=0;
			$r_sql="select * from result where s_id='$s_id' ";
			...
			{
?>
		<a href='sem_search.php?s_id=<?php echo $s_id;?>& year=<?php echo $i;?> ' class='button' style="margin-left:100px;"> <?php echo $i; ?></a></br></br></br>
<?php }} ?>
</div>
...
<div style="border:0px solid black;width:750px;float:left;">
 ...
 <table width="740px" style="border: 2px solid gray;margin-bottom:20px;font-size:11px;" align="center" >
		...
		<?php
		//Print the contents
		$r_sql="select * from result where s_id='$s_id' AND status='active' order by id desc ";
		$r_res=mysql_query($r_sql);
		$c=0;
		$fl=0;
		...
		$c++;?>
			<tr style="text-align:center;">
				...
				<td><a href="sea_res_view.php?id=<?php echo $row['id'];?>&ids=<?php echo $s_id;?>">view</a> </td>
			</tr>
...
<?php
		...
		<h1 style="margin-top:200px;">"Student ID is inactive"</h1>
		<?php } 
		else
		{ ?>
		<h1 style="margin-top:200px;">"No student founded"</h1>
		<?php }?>
